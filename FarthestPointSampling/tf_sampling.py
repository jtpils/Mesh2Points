...
def farthest_points_sampling(h5files):
    ...
    with tf.device('/gpu:0'):
        for h5file in h5files:
            ...
            print(hd6_data.shape, '-->', ret.shape, ret.dtype)


def FPS(points, snum, save_fn):
    """Farthest Points Sampling, operation
        points: n*3 or b*n*3
        snum: 2048, 4096, num of sampling points
        save_fn: save file name
    """
    with tf.device('/gpu:0'):
        pts_data = np.array(points).astype('float32')
        if len(pts_data.shape) < 3:
            pts_data = np.expand_dims(pts_data, 0)
        pt_sample = tf.Variable(pts_data)
        # pdb.set_trace()
        reduced_sample = gather_point(pt_sample, farthest_point_sample(snum, pt_sample))
        out_h5file = save_fn + '.hd5'
        with tf.Session('') as sess:
            sess.run(pt_sample.initializer)
            ret = sess.run(reduced_sample)
            # print(ret.shape)
            save_data_to_h5(ret, out_h5file)
        print(pts_data.shape, '-->', ret.shape, ret.dtype)
        return ret
